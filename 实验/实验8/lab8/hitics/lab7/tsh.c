...
/*void sigchld_handler(int sig) {//传入的形参是信号sig
    ...
    int mysig;
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {//立即返回，若等待集合中的子进程中都没有停止或终止
        if (WIFEXITED(status)) {//若引起进程返回的原因是子进程调用exit或者return正常终止
            deletejob(jobs, pid);//将pid所指的job从job结构数组中删除
        }
        if (WIFSIGNALED(status)) {//若引起进程返回的原因是子进程未捕捉信号终止
            struct job_t *tjob;
            tjob = getjobpid(jobs, pid);
            mysig = WTERMSIG(status);
            printf("Job [%d] (%d) terminated by signal %d\n", tjob->jid, pid, mysig);
            deletejob(jobs, pid);
        }
        if (WIFSTOPPED(status)) {//若引起进程返回的原因是子进程状态是停止的
            struct job_t *sjob;//获取此子进程pid所在的job
            sjob = getjobpid(jobs, pid);
            mysig = WSTOPSIG(status);
