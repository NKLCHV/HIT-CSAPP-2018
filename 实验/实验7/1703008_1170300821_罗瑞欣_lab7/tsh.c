...
void sigchld_handler(int sig) {//传入的形参是信号sig
	int status,olderrno=errno;//为防止错误的信号处理函数修改，用errnoolderrno保存原来的errno
	pid_t pid;//保存引起进程终止的子进程pid
	sigset_t mask_all,prev_all;//mask_all保存所有的信号，pre_all保存原来信号阻塞向量中的所有信号
...
void clearjob(struct job_t *job) {//将job数组中的一个job初始化
	job->pid = 0;//将job的pid置为0
	job->jid = 0;//将job的jid置为0
	job->state = UNDEF;//将job的state置为UNDEF
	job->cmdline[0] = '\0';//将job的cmdline置为空
}
/* initjobs - Initialize the job list */
void initjobs(struct job_t *jobs) {//传入的形参是jobs数组
	int i;
	for (i = 0; i < MAXJOBS; i++)//for循环将每一个job初始化
		clearjob(&jobs[i]);//一次初始化一个job
}

/* maxjid - Returns largest allocated job ID */
int maxjid(struct job_t *jobs)//传入的形参是jobs数组
{
	int i, max=0;
	for (i = 0; i < MAXJOBS; i++)//循环比较找到最大的jid
		if (jobs[i].jid > max)
			max = jobs[i].jid;
	return max;//返回值是最大的jid
}

/* addjob - Add a job to the job list */
int addjob(struct job_t *jobs, pid_t pid, int state, char *cmdline) {//形参是jobs数组和新加入job的信息
	int i;

	if (pid < 1)//pid小于错误，return 0返回
		return 0;

	for (i = 0; i < MAXJOBS; i++) {//找到未被占用的job进行赋值
		if (jobs[i].pid == 0) {
			jobs[i].pid = pid;//将pid赋值给空job
			jobs[i].state = state;//将state赋值给空job
			jobs[i].jid = nextjid++;//nextjid得到此job的jid
			if (nextjid > MAXJOBS)//判断job是否满，即jid超过了要求的最大值，
				nextjid = 1;
			strcpy(jobs[i].cmdline, cmdline);//复制新的命令行到新job的cmdline
			if (verbose) {//打印输出新加的job信息
...
int deletejob(struct job_t *jobs, pid_t pid) {//形参是所有的job和要删除的job的pid
	int i;
	...
	for (i = 0; i < MAXJOBS; i++) {
		if (jobs[i].pid == pid) {//遍历找到删除的job
			clearjob(&jobs[i]);//初始化即删除
			nextjid = maxjid(jobs) + 1;//jid++
			return 1;//删除成功
		}
	}
	return 0;//删除失败
...
pid_t fgpid(struct job_t *jobs) {
	...
	for (i = 0; i < MAXJOBS; i++)
		if (jobs[i].state == FG)
			return jobs[i].pid;//遍历找到前台进程，并返回前台进程的pid
	return 0;
}
...
struct job_t *getjobpid(struct job_t *jobs, pid_t pid) {
	...
	if (pid < 1)//pid小于1错误，返回空
		return NULL;
	for (i = 0; i < MAXJOBS; i++)
		if (jobs[i].pid == pid)//遍历寻找pid对应的进程
			return &jobs[i];//并返回该进程job
	return NULL;
}

/* getjobjid  - Find a job (by JID) on the job list */
struct job_t *getjobjid(struct job_t *jobs, int jid) {//形参是所有job，以及查找的job的jid
	int i;

	if (jid < 1)//jid小于1错误，返回空
		return NULL;
	for (i = 0; i < MAXJOBS; i++)
		if (jobs[i].jid == jid)//遍历找jid对应的进程
			return &jobs[i];//返回该进程job
	return NULL;
}
...
int pid2jid(pid_t pid) {
	...
	for (i = 0; i < MAXJOBS; i++)
		if (jobs[i].pid == pid) {//遍历寻找pid对应的进程
			return jobs[i].jid;//返回该进程job的jid
		}
	return 0;
}
...
void listjobs(struct job_t *jobs) {
	...
	for (i = 0; i < MAXJOBS; i++) {
		if (jobs[i].pid != 0) {//遍历输出job信息
			printf("[%d] (%d) ", jobs[i].jid, jobs[i].pid);
			switch (jobs[i].state) {//根据job运行情况输出对应语句
				case BG://BG代表后台运行
					printf("Running ");
					break;
				case FG://FG代表前台运行
 ...
 */
handler_t *Signal(int signum, handler_t *handler) {//信号处理可移植
